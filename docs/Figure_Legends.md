...
**(A)** Pairwise Spearman rank correlations among Days to 50% Flowering (DTFlower), Days to 50% Ripe Fruit (DTFruit), Fruiting Period (Flow2Fruit), and 25-Fruit Weight (FruitWeight) for each year (2023, 2024, 2025). Color scale indicates correlation coefficient (blue = positive, red = negative). Notable shifts between years reflect the frost-compression of DTFlower variance in 2025 (DTFlower–DTFruit r: 0.657 in yr.23 → 0.344 in yr.25). **(B)** Count of frost days (minimum temperature ≤ 32°F) and hard-freeze events (minimum temperature ≤ 28°F) per year at the Georgia field station. Year 2025 had 3.5× more frost days and 6× more hard-freeze events than 2023. **(C)** Phenotypic standard deviation of DTFlower BLUEs by year (2023: 13.3 days; 2024: 11.2 days; 2025: 6.9 days), illustrating the 48% reduction in variance driven by frost-synchronization of flowering in 2025. BLUEs computed via REML mixed model (lmer: trait ~ Year + [1|Genotype]) across approximately 800 Southern Highbush Blueberry accessions.
